# 🎵 电脑音乐软件汇总

## 📥 资源下载
#### **软件下载（百度网盘）**：[点击跳转保存](https://pan.baidu.com/s/1NUNv-j0kghfhaGep4myk0Q?pwd=52pj)
---
### 1. MusicFree(桌面版)
* 地址1： `https://musicfreepluginshub.2020818.xyz/plugins.json`。
* 地址2： `https://fastly.jsdelivr.net/gh/Huibq/keep-alive/Music_Free/myPlugins.json`

使用方法：`设置` -> `插件管理` -> `从网络安装插件` -> `填入地址`
### 2. 洛雪音乐LX Music
使用方法：`设置`-`基本设置`-`自定义源管理`-`导入`-`在线导入`-`填入链接`
#### 插件接口链接：
* **Huibg源：**`https://fastly.jsdelivr.net/gh/Huibq/keep-alive/render_api.js`
* **野花：**`https://fastly.jsdelivr.net/gh/Huibq/keep-alive/Music_Free/myPlugins.json`
* **六音：**`https://ghproxy.net/raw.githubusercontent.com/pdone/lx-music-source/main/sixyin/latest.js`
* **野草：**`https://ghproxy.net/raw.githubusercontent.com/pdone/lx-music-source/main/grass/latest.js`
* **ikun音源：**`https://ghproxy.net/raw.githubusercontent.com/pdone/lx-music-source/main/ikun/latest.js`
### 3. Lofi-Radio-Player
一款逼格非常高的极简 Lofi 音乐电台软件，适合学习工作背景音。
### 4. 千千静听
经典中的经典，老牌本地音乐播放器。
### 5. 在线听歌/歌曲下载网站
* ① GD音乐台：[https://music.gdstudio.org](https://music.gdstudio.org)
* ② 无损音乐下载：[https://flac.music.hi.cn](https://flac.music.hi.cn)
